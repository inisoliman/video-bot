import telebot
   ...
   bot = telebot.TeleBot(BOT_TOKEN, skip_pending=True)  # Add skip_pending=True
   ...
   def list_videos(message):
       categories = set(video[4] for video in get_videos())
   ...
   def callback_query(call):
       if call.data.startswith("video_"):
           ...
           try:
               ...
               bot.reply_to(call.message, f"Error sending video: {str(e)}")
       elif call.data.startswith("cat_"):
           category = call.data.replace("cat_", "")
           videos = get_videos(category)
           if not videos:
               bot.reply_to(call.message, f"No videos in category '{category}'.")
               return
           keyboard = InlineKeyboardMarkup()
           for video in videos:
               caption = video[1] or video[3] or "No title"
               keyboard.add(InlineKeyboardButton(text=caption[:50], callback_data=f"video_{video[0]}_{video[2]}"))
           bot.reply_to(call.message, f"Videos in '{category}':", reply_markup=keyboard)
   ...
   def handle_new_video(message):
       if message.chat.type == 'supergroup' or message.chat.type == 'channel':
           if str(message.chat.id) == CHANNEL_ID:
               ...
               time.sleep(1)
               bot.reply_to(message, f"New video added with ID: {message.message_id}")
